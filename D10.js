...
console.log(isThisAnEmail("alessiacampanile14@gmail.com"));
console.log(isThisAnEmail("alessiacampanile14gmail.com"));
...
function isTodayMyBirthday(data) {
  ...
  const oggi = new Date();
  if (compleanno.getDay() === oggi.getDay() && compleanno.getMonth() === oggi.getMonth()) {
    return true;
  } else {
    return false;
  }
}
console.log(isTodayMyBirthday("2002-12-22"));
console.log(isTodayMyBirthday("2002-10-18"));
...
function deleteProp(string, oggetto) {
  delete oggetto[string];
  return oggetto;
}
let user = { nome: "Alessia", cognome: "Campanile" };
console.log(deleteProp("cognome", user));
...
function newestMovie() {
  let oggettoConfronto = {
    Year: "0",
  };
  for (let i = 0; i < movies.length; i++) {
    if (oggettoConfronto.Year < movies[i].Year) {
      oggettoConfronto = movies[i];
    }
  }

  return oggettoConfronto;
}
...
function onlyInLastMillennium() {
  let anni = [];
  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Year < 2000) anni.push(movies[i].Year);
  }
...
function searchByTitle(string) {
  let filmTrovati = [];
  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Title.includes(string)) filmTrovati.push(movies[i]);
  }
